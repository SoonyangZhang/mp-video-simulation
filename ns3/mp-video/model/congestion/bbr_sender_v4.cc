...
MyBbrSenderV4::MyBbrSenderV4(uint64_t min_bps,BandwidthObserver *observer)
...
MyBbrSenderV4::~MyBbrSenderV4(){
}
QuicBandwidth MyBbrSenderV4::PaceRate(){
	if(pacing_rate_==QuicBandwidth::Zero()){
		return high_gain_*min_bw_;
	}
	return pacing_rate_;
}
QuicBandwidth MyBbrSenderV4::GetReferenceRate(){
	if(pacing_rate_==QuicBandwidth::Zero()){
		return min_bw_;
	}
	return pacing_rate_;
}
QuicBandwidth MyBbrSenderV4::BandwidthEstimate(){
	return max_bandwidth_.GetBest();
}
bool MyBbrSenderV4::ShouldSendProbePacket(){
	return true;
}
void MyBbrSenderV4::OnPacketSent(QuicTime event_time,
...
void MyBbrSenderV4::OnAck(QuicTime event_time,
		QuicPacketNumber packet_number){
...
void MyBbrSenderV4::EnterStartUpMode(){
	mode_=ST_START;
	pacing_gain_=high_gain_;
}
void MyBbrSenderV4::CheckIfFullBandwidthReached(){
...
bool MyBbrSenderV4::CheckIfCongestion(){
...
void MyBbrSenderV4::EnterIncreaseMode(QuicTime now){
	mode_=ST_INCREASE;
...
void MyBbrSenderV4::MaybeEnterOrExitDecrease(QuicTime now,
...
QuicTime::Delta MyBbrSenderV4::GetMinRtt() {
  return !min_rtt_.IsZero() ? min_rtt_ :QuicTime::Delta::FromMilliseconds(200);
}
QuicByteCount MyBbrSenderV4::GetTargetCongestionWindow(float gain){
...
QuicByteCount MyBbrSenderV4::GetTargetInflightInDecrease(float gain){
...
void MyBbrSenderV4::UpdateGainCyclePhase(QuicTime now){
...
void MyBbrSenderV4::CalculatePacingRate(){
...
bool MyBbrSenderV4::UpdateRoundTripCounter(QuicPacketNumber last_acked_packet){
...
void MyBbrSenderV4::AddPacketInfo(QuicTime now,QuicPacketNumber packet_number,QuicByteCount payload){
	std::shared_ptr<PerPacket> packet(new PerPacket(now,payload));
	sent_packets_map_.insert(std::make_pair(packet_number,packet));
}
void MyBbrSenderV4::UpdateRttAndInflight(QuicTime now,
		QuicPacketNumber packet_number){
...
void MyBbrSenderV4::UpdateMaxBw(){
	QuicBandwidth bw=BandwidthEstimate();
	if(bw>max_bw_in_increase_){
		max_bw_in_increase_=bw;
	}
}
std::string MyBbrSenderV4::GetStateString(){
	if(mode_==ST_START){
...
void MyBbrSenderV4::PrintDebugInfo(uint64_t bps,std::string state){
	if(!trace_state_cb_.IsNull()){
		trace_state_cb_(bps,state);
	}
}
void MyBbrSenderV4::UpdateCongestionSignal(QuicPacketNumber ack_seq){
