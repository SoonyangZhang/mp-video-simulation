...
const int64_t TPacedSender::kMaxQueueLengthMs = 2000;
const float TPacedSender::kDefaultPaceMultiplier = 2.5f;

TPacedSender::TPacedSender(const webrtc::Clock* clock,
		webrtc::PacedSender::PacketSender* packet_sender,
		webrtc::RtcEventLog* event_log) :
    TPacedSender(clock, packet_sender, event_log,
                webrtc::field_trial::IsEnabled("WebRTC-RoundRobinPacing")
                    ? rtc::MakeUnique<webrtc::PacketQueue2>(clock)
                    : rtc::MakeUnique<webrtc::PacketQueue>(clock)) {}
...
TPacedSender::TPacedSender(const webrtc::Clock* clock,
    ...
    : clock_(clock),
      ...
      alr_detector_(rtc::MakeUnique<webrtc::AlrDetector>()),
      paused_(false),
      media_budget_(rtc::MakeUnique<webrtc::IntervalBudget>(0)),
      padding_budget_(rtc::MakeUnique<webrtc::IntervalBudget>(0)),
      prober_(rtc::MakeUnique<webrtc::BitrateProber>(event_log)),
...
TPacedSender::~TPacedSender() {}

void TPacedSender::CreateProbeCluster(int bitrate_bps) {
  rtc::CritScope cs(&critsect_);
  prober_->CreateProbeCluster(bitrate_bps, clock_->TimeInMilliseconds());
}

void TPacedSender::Pause() {
  {
    rtc::CritScope cs(&critsect_);
    if (!paused_)
      RTC_LOG(LS_INFO) << "TPacedSender paused.";
    paused_ = true;
...
void TPacedSender::Resume() {
  {
    rtc::CritScope cs(&critsect_);
    if (paused_)
      RTC_LOG(LS_INFO) << "TPacedSender resumed.";
    paused_ = false;
...
void TPacedSender::SetProbingEnabled(bool enabled) {
  rtc::CritScope cs(&critsect_);
  RTC_CHECK_EQ(0, packet_counter_);
  prober_->SetEnabled(enabled);
}

void TPacedSender::SetEstimatedBitrate(uint32_t bitrate_bps) {
  if (bitrate_bps == 0)
    RTC_LOG(LS_ERROR) << "TPacedSender is not designed to handle 0 bitrate.";
  rtc::CritScope cs(&critsect_);
...
void TPacedSender::SetSendBitrateLimits(int min_send_bitrate_bps,
                                       int padding_bitrate) {
  rtc::CritScope cs(&critsect_);
...
void TPacedSender::InsertPacket(webrtc::RtpPacketSender::Priority priority,
                               uint32_t ssrc,
  ...
  packets_->Push(webrtc::PacketQueue::Packet(priority, ssrc, sequence_number,
                                     capture_time_ms, now_ms, bytes,
                                     retransmission, packet_counter_++));
}

void TPacedSender::SetAccountForAudioPackets(bool account_for_audio) {
  rtc::CritScope cs(&critsect_);
  account_for_audio_ = account_for_audio;
}

int64_t TPacedSender::ExpectedQueueTimeMs() const {
  rtc::CritScope cs(&critsect_);
...
rtc::Optional<int64_t> TPacedSender::GetApplicationLimitedRegionStartTime()
    const {
  rtc::CritScope cs(&critsect_);
  return alr_detector_->GetApplicationLimitedRegionStartTime();
}

size_t TPacedSender::QueueSizePackets() const {
  rtc::CritScope cs(&critsect_);
  return packets_->SizeInPackets();
}

int64_t TPacedSender::FirstSentPacketTimeMs() const {
  rtc::CritScope cs(&critsect_);
  return first_sent_packet_ms_;
}

int64_t TPacedSender::QueueInMs() const {
  rtc::CritScope cs(&critsect_);
...
int64_t TPacedSender::TimeUntilNextProcess() {
  rtc::CritScope cs(&critsect_);
...
void TPacedSender::Process() {
  ...
  while (!packets_->Empty()) {
    ...
    const webrtc::PacketQueue::Packet& packet = packets_->BeginPop();
...
void TPacedSender::ProcessThreadAttached(webrtc::ProcessThread* process_thread) {
  RTC_LOG(LS_INFO) << "ProcessThreadAttached 0x" << std::hex << process_thread;
  process_thread_ = process_thread;
}

bool TPacedSender::SendPacket(const webrtc::PacketQueue::Packet& packet,
                             const webrtc::PacedPacketInfo& pacing_info) {
  RTC_DCHECK(!paused_);
  if (media_budget_->bytes_remaining() == 0 &&
      pacing_info.probe_cluster_id == webrtc::PacedPacketInfo::kNotAProbe) {
    return false;
  }
  ...
  if (success) {
    if (packet.priority != webrtc::RtpPacketSender::Priority::kHighPriority|| account_for_audio_) {
...
size_t TPacedSender::SendPadding(size_t padding_needed,
                                const webrtc::PacedPacketInfo& pacing_info) {
...
void TPacedSender::UpdateBudgetWithElapsedTime(int64_t delta_time_ms) {
  media_budget_->IncreaseBudget(delta_time_ms);
  padding_budget_->IncreaseBudget(delta_time_ms);
}

void TPacedSender::UpdateBudgetWithBytesSent(size_t bytes_sent) {
  media_budget_->UseBudget(bytes_sent);
  padding_budget_->UseBudget(bytes_sent);
}

void TPacedSender::SetPacingFactor(float pacing_factor) {
  rtc::CritScope cs(&critsect_);
...
float TPacedSender::GetPacingFactor() const {
  rtc::CritScope cs(&critsect_);
  return pacing_factor_;
}

void TPacedSender::SetQueueTimeLimit(int limit_ms) {
  rtc::CritScope cs(&critsect_);
